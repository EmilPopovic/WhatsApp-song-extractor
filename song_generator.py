...
from youtube import YtExtractor
...
class SongGenerator:
    ...
    def set_source(self) -> None:
        ...
        try:
            yt_info = YtExtractor.yt_get(f'{self.author.name} {self.name}')[0]
        except:
            self.is_good = False
            return
    ...
    def short(self) -> str:
        return f'{self.sender},{self.author.name},{self.name},{self.spotify_link},{self.yt_link}'

    def __repr__(self) -> str:
        return f'sender: {self.sender} | author: {self.author.name} | name: {self.name} | spotify: {self.spotify_link} | yt: {self.yt_link}'
